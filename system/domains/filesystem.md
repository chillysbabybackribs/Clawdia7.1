...
- whether the current directory matters for the operation
...
**Important:** Shell commands do not share session state between separate calls. Do not assume that a `cd` or exported variable from one `shell_exec` call persists into the next. Use absolute paths and self-contained commands.
...
These actions require stronger grounding and verification before execution.
...
- assumed shell state carried across calls (it does not)
...
→ Active task file: `tasks/file-edit.md` or `tasks/shell-task.md`
