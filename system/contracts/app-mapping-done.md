...
- It is clear whether the map covers one screen, one workflow, one dialog, one application region, or a broader multi-step area.
- Do not imply full-app coverage if only a narrow area was mapped.

### 3. The map is grounded in observed UI structure
The map must be tied to actual observed application state such as:
...
At least one validation pass must confirm the produced map corresponds to current application state.

Acceptable validation:
- re-checking the same UI surface
- confirming mapped controls are still present via `a11y_find` or screenshot
- confirming region relationships
- confirming the map was not invalidated by a modal, zoom, layout shift, or step change
- confirming the map matches the app state it claims to describe
